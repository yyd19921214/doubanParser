...
public class MovieinfoDAO extends BaseHibernateDAO {
	private static final Log log = LogFactory.getLog(MovieinfoDAO.class);
	// property constants
	...
	public Movieinfo findById(java.lang.Integer id) {
		...
		try {
			Movieinfo instance = (Movieinfo) getSession().get("dao.Movieinfo",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Movieinfo instance) {
		...
		try {
			List results = getSession().createCriteria("dao.Movieinfo")
	...
	public List findByMovieCategory(Object movieCategory) {
		return findByProperty(MOVIE_CATEGORY, movieCategory);
	}
