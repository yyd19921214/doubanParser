...
public class BookinfoDAO extends BaseHibernateDAO {
	private static final Log log = LogFactory.getLog(BookinfoDAO.class);
	// property constants
	...
	public Bookinfo findById(java.lang.Integer id) {
		...
		try {
			Bookinfo instance = (Bookinfo) getSession().get("dao.Bookinfo", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Bookinfo instance) {
		...
		try {
			List results = getSession().createCriteria("dao.Bookinfo")
	...
	public List findByBookCategory(Object bookCategory) {
		return findByProperty(BOOK_CATEGORY, bookCategory);
	}
